...
#include "gd32_adc_inputs.inc"
#include "gd32_pwm_inputs.inc"
...
{
  bool success = gd32_hal_adc_init(_ADC_adc, n_ADC, _ADC_inputs, _ADC_GPIOs, n_GPIO);
#if defined(ADC_SPI)
...
{
  bool success = gd32_hal_adc_start_read(_ADC_adc, n_ADC, _ADC_inputs, n_inputs);
#if defined(ADC_SPI)
  if (n_ADC_spi > 0) {
...
#endif
  gd32_hal_adc_wait_completion(_ADC_adc, n_ADC, _ADC_inputs, n_inputs);
}
...
const etx_hal_adc_driver_t _adc_driver = {
  ...
  .set_input_mask = gd32_hal_set_inputs_mask,
  .get_input_mask = gd32_hal_get_inputs_mask,
};
